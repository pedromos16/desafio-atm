...
describe('calcularCedulas', () => {
  ...
  test('Deve lançar um erro para um valor que não pode ser atendido', () => {
  ...
  test('Deve retornar a quantidade correta de cédulas para um valor muito grande', () => {
    const resultado = calcularCedulas(9876);
  ...
  test('Deve lançar erro para valores não inteiros', () => {
    expect(() => calcularCedulas(150.5)).toThrow('Valor não pode ser atendido com as cédulas disponíveis.');
  });

  test('Deve retornar nenhuma cédula quando o valor é múltiplo de uma única denominação', () => {
    const resultado = calcularCedulas(200);
    expect(resultado).toEqual({ 100: 2, 50: 0, 20: 0, 10: 0, 5: 0, 2: 0 });
  });
});
